...
openssl req -new -x509 -days 365 -key $CERT_DIR/ca.key -out $CERT_DIR/ca.crt \
  -subj "/C=US/ST=CA/L=San Francisco/O=Zero Trust PoC/CN=Zero Trust CA"

echo "Generating server certificate and key..."
openssl genrsa -out $CERT_DIR/server.key 2048
openssl req -new -key $CERT_DIR/server.key -out $CERT_DIR/server.csr \
  -subj "/C=US/ST=CA/L=San Francisco/O=Zero Trust PoC/CN=api-gateway"
...
openssl req -new -key $CERT_DIR/client.key -out $CERT_DIR/client.csr \
  -subj "/C=US/ST=CA/L=San Francisco/O=Zero Trust PoC/CN=client"
...
openssl req -new -key $CERT_DIR/opa.key -out $CERT_DIR/opa.csr \
  -subj "/C=US/ST=CA/L=San Francisco/O=Zero Trust PoC/CN=opa"
