...
openssl req -new -x509 -days 365 -key $CERT_DIR/ca.key -out $CERT_DIR/ca.crt \
  -subj "/C=NG/ST=Lagos/L=Lagos/O=Zero Trust/CN=Zero Trust CA"
...
openssl req -new -key $CERT_DIR/client.key -out $CERT_DIR/client.csr \
  -subj "/C=NG/ST=Lagos/L=Lagos/O=Zero Trust/CN=client"
...
openssl req -new -key $CERT_DIR/opa.key -out $CERT_DIR/opa.csr \
  -subj "/C=NG/ST=Lagos/L=Lagos/O=Zero Trust/CN=opa"
